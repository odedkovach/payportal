...
const COLORS = ['#b91c1c', '#6b7280', '#9ca3af', '#e5e7eb']; // Wetherby Red, Dark Grey, Light Grey, Off-white
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-6">
                <div>
                    <h2 className="text-3xl font-bold text-gray-900 tracking-tight flex items-center gap-3">
                        <span className="w-2 h-8 bg-red-700 rounded-sm"></span>
                        {data.title}
                    </h2>
                    <p className="text-gray-500 mt-1 ml-5">
                        <span className="font-semibold text-gray-700">Period:</span> {data.period} •
                        <span className="ml-2 text-xs bg-red-50 text-red-700 px-2 py-0.5 rounded-full border border-red-100 uppercase font-bold tracking-wider">
                            Phase 1 Analysis
                        </span>
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button className="flex items-center gap-2 px-4 py-2 bg-red-700 text-white rounded-lg text-sm font-medium hover:bg-red-800 transition-colors shadow-sm hover:shadow-md">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer>
                            <BarChart data={data.aging} layout="vertical" margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                ...
                                />
                                <Bar dataKey="value" fill="#b91c1c" radius={[0, 6, 6, 0]} barSize={32}>
                    ...
                    <div className="mt-4 p-3 bg-red-50 border border-red-100 rounded-lg flex items-start gap-3">
                        <IconAlertTriangle className="w-5 h-5 text-red-600 mt-0.5 shrink-0" />
                            ...
                            {data.discrepancies.map((item) => (
                                <tr key={item.id} className="hover:bg-red-50/10 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${item.status === 'Overdue' ? 'bg-red-100 text-red-800 border-red-200' :
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                                        <button
                                            ...
                                            className="text-red-600 hover:text-red-900 font-medium text-xs border border-red-200 bg-red-50 hover:bg-red-100 px-3 py-1 rounded transition-colors"
                                        >
                                            Chase Payment
                                        </button>
                                    </td>
...
const KPICard = ({ label, value, subtext, icon, valueColor = "text-gray-900", overrideIconColor, highlight = false }: any) => (
    <div className={`p-6 rounded-xl border shadow-sm transition-all duration-300 hover:shadow-md ${highlight ? 'bg-white border-red-200 shadow-red-50' : 'bg-white border-gray-200'
        }`}>
        <div className="flex justify-between items-start mb-4">
            <span className="text-sm font-medium text-gray-500">{label}</span>
            <div className={`p-2 rounded-lg ${highlight ? 'bg-red-50' : 'bg-gray-50'} ${overrideIconColor || ''}`}>
                {icon}
            </div>
        </div>
